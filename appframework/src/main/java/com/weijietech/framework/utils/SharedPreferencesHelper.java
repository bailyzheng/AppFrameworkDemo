...
/**
 * Created by eagle on 2016-12-11.
 */
        ...
        for (String key : keys) {
            ...
            try {
                if (value == null) {
                    ...
                    TLog.e(TAG, String.format("Con't support save this type:%s, value:%s, key:%s",
                            type, value, key));
                }
            } catch (IllegalArgumentException e) {
                TLog.e(TAG, "Save error:" + e.getMessage());
            }
        }
...
//     */
    ...
    private static <T> void buildValuesToMap(Class<?> clx, T t, String preFix,
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                TLog.e(TAG, "buildValuesToMap error:" + e.getMessage());
                continue;
            }
        ...
        for (Field field : fields) {
            ...
            if (value != null) {
                try {
                    ...
                    e.printStackTrace();
                    TLog.e(TAG, String.format("Set field error, Key:%s, type:%s, value:%s",
                            key, fieldType, value));
                }
            } else {
                TLog.e(TAG, String.format("Get field value error, Key:%s, type:%s",
                        key, fieldType));
            }
        }
    ...
    public static String getString(Context context, String name, String key, String defultValue) {
        SharedPreferences sp = context.getSharedPreferences(name, Context.MODE_PRIVATE);
        return sp.getString(key, defultValue);
    }

    public static int getInt(Context context, String name, String key, int defultValue) {
        SharedPreferences sp = context.getSharedPreferences(name, Context.MODE_PRIVATE);
        return sp.getInt(key, defultValue);
    }

    public static float getFloat(Context context, String name, String key, float defultValue) {
        SharedPreferences sp = context.getSharedPreferences(name, Context.MODE_PRIVATE);
        return sp.getFloat(key, defultValue);
    }

    public static boolean getBoolean(Context context, String name, String key, boolean defultValue) {
        SharedPreferences sp = context.getSharedPreferences(name, Context.MODE_PRIVATE);
